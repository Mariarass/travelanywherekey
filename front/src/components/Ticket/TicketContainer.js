...
class TicketContainers extends React.Component {
    ...
    changeTicket = (city,where) => {
        ...
        if (this.props.language === 'ru') {
        axios.get(`http://localhost:5000/api/tour?city=${city}&where=${where}`).then(response => {
     
        this.props.setTicket(response.data)
        })}

        if (this.props.language === 'en') {
            axios.get(`http://localhost:5000/api/tourEng?city=${city}&where=${where}`).then(response => {
